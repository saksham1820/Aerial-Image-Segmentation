...
def evaluate(model, data_loader, loss_fn):
    ...
    print("Evaluation Loss {:.3f} | IoU Building:{:.3f} ".format(total_loss/len(data_loader), total_iou_building/len(data_loader)))
    return total_loss/len(data_loader), total_iou_building/len(data_loader)
...
if __name__ == "__main__":
    ...
    val_dataset = AerialImage("val", resized_shape=resized_shape, root_dir=ROOT_DIR)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        # val_loss, iou_bulding = evaluate(model, val_loader, loss_fn)
